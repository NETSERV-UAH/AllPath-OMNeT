...
Define_Module(LearningTableNewWAPB);

std::ostream& operator<<(std::ostream& os, const LearningTableNewWAPB::AddressEntry& entry)
{
...
LearningTableNewWAPB::LearningTableNewWAPB()
{
...
void LearningTableNewWAPB::initialize()
{
...
void LearningTableNewWAPB::handleMessage(cMessage *)
{
...
LearningTableNewWAPB::AddressTable *LearningTableNewWAPB::getTableForVid(unsigned int vid)
{
    if (vid == 0)
        return addressTable;
...
int LearningTableNewWAPB::getPortForAddress(MACAddress& address, unsigned int vid)
{
    Enter_Method("LearningTableNewWAPB::getPortForAddress()");
...
MACAddress LearningTableNewWAPB::getNextHopForAddress(MACAddress address, unsigned int vid)
{
    Enter_Method("LearningTableNewWAPB::getPortForAddress()");
...
bool LearningTableNewWAPB::updateTableWithAddress(int portno, MACAddress& address, int status, unsigned int vid)
{
    Enter_Method("LearningTableNewWAPB::updateTableWithAddress()");
...
//used for wireless ARP-Path
bool LearningTableNewWAPB::updateTableWithAddress(MACAddress nextHop, MACAddress& address, int status, unsigned int vid)
{
    Enter_Method("LearningTableNewWAPB::updateTableWithAddress()");
    if (address.isBroadcast())
        return false;
...
//used for wireless ARP-Path
bool LearningTableNewWAPB::refreshTimer(MACAddress& address, unsigned int vid)
{
    Enter_Method("LearningTableNewWAPB::refreshTimer()");
    if (address.isBroadcast())
        return false;
    ...
    if (iter == table->end()) {
        ...
        // Update existing entry
        EV << "refreshing entry timer in Learning Table: " << address << "\n";
        AddressEntry& entry = iter->second;
        entry.insertionTime = simTime();
    }
    return true;
...
void LearningTableNewWAPB::flush(int portno)
{
    Enter_Method("LearningTableNewWAPB::flush():  Clearing gate %d cache", portno);
...
void LearningTableNewWAPB::printState()
{
...
void LearningTableNewWAPB::copyTable(int portA, int portB)
{
    for (auto & elem : vlanAddressTable) {
        AddressTable *table = elem.second;
        for (auto & table_j : *table)
...
void LearningTableNewWAPB::removeAgedEntriesFromVlan(unsigned int vid)
{
    AddressTable *table = getTableForVid(vid);
    if (table == nullptr)
        return;
...
void LearningTableNewWAPB::removeAgedEntriesFromAllVlans()
{
    for (auto & elem : vlanAddressTable) {
        AddressTable *table = elem.second;
        // TODO: this part could be factored out
...
void LearningTableNewWAPB::removeAgedEntriesIfNeeded()
{
...
void LearningTableNewWAPB::clearTable()
{
    for (auto & elem : vlanAddressTable)
        delete elem.second;
...
void LearningTableNewWAPB::setAgingTime(simtime_t agingTime)
{
    this->agingTime = agingTime;
}

void LearningTableNewWAPB::resetDefaultAging()
{
    agingTime = par("agingTime");
}
