...
Define_Module(MACRelayUnitWAPB);

//Initialize variables at simulation start.
void MACRelayUnitWAPB::initialize(int stage)
{
    /*MACRelayUnit.h inet3.6.3*/
    ...
    blockingTime = getModuleByPath(par("blockingTablePath"))->par("blockingTime");  //blockingTime = par("blockingTime");
...
void MACRelayUnitWAPB::handleMessage(cMessage *msg)
{
    EV << "->MACRelayUnitWAPB::handleMessage()" << endl;
    if (!isOperational) {
...
void MACRelayUnitWAPB::handleAndDispatchFrame(EtherFrame *frame)
{
...
void MACRelayUnitWAPB::start()
{
    //addressTable->clearTable();
    isOperational = true;
}

void MACRelayUnitWAPB::stop()
{
    //addressTable->clearTable();
    isOperational = true;
}

bool MACRelayUnitWAPB::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void MACRelayUnitWAPB::handleAndDispatchFrameV2(EtherFrame *frame, int inputport)
{
    ...
    if(!strcmp("arpREQ",frame->getName()))
    {
        ...
        AddressEntry entry;
        entryExists = addressTable->findEntry(srcAddress, entry);  //entryExists = findEntry(srcAddress, entry);
        if (entryExists)
        {
            if(entry.status == locked)
            {
                ...
                if (inputport == entry.port)
                {
                    ...
                    addressTable->updateTable(srcAddress, entry);
            ...
            else
            {
                ...
                addressTable->addEntry(entry, srcAddress, inputport, protocolVersion);
    ...
    else if(!strcmp("arpREPLY",frame->getName()))
    {
        ...
        if (entryExists)
        {
            if(entry.status == locked)
            {
                ...
                {
                    entry.inTime = simTime(); //Refresco
                    addressTable->updateTable(srcAddress, entry);  //in previous version : addressTable[srcAddress] = entry;
                }
        ...
        entryExists = addressTable->findEntry(dstAddress, destEntry); //entryExists = findEntry(dstAddress, destEntry);
        if (entryExists)
        {
    ...
    else if(dstAddress.isBroadcast() || dstAddress.isMulticast())
    {
        ...
        else
        {
            ...
            entryExists = addressTableBasic->findEntryBasic(srcAddress, entryBasic);  //entryExists = findEntryBasic(srcAddress, entryBasic);
            if (!entryExists) //Not found, add address to bcast table
            {
                EV << "      Entry not found -> Add new 'broadcast' entry..." << endl;
                addressTableBasic->addEntryBasic(entryBasic, srcAddress, inputport);//addEntryBasic(entryBasic, srcAddress, inputport);
            ...
            else //Address found
            {
                ...
                if (entryBasic.port == inputport) //If port is coincident -> refresh entry
                {
                    ...
                    addressTableBasic->updateTableBasic(srcAddress, entryBasic);  //addressTableBasic[srcAddress] = entryBasic;
                    broadcastFrame(frame,inputport);
                }
                else //Else -> discard
                {
...
void MACRelayUnitWAPB::handleAndDispatchFrameV3(EtherFrame* frame, int inputport)
{
    ...
    else if(!strcmp("arpREPLY",frame->getName()))
    {
        ...
        //Learning
        AddressEntry entry;
        bool entryExists = addressTable->findEntry(srcAddress, entry);  //entryExists = findEntry(srcAddress, entry);
        if (entryExists)
        {
            if(entry.status == locked)
            {
    ...
    else if(dstAddress.isBroadcast() || dstAddress.isMulticast())
    {
        ...
        else if(dstAddress.isMulticast() && multicastActive)
        {
            ...
            else//SI LA TRAMA ES MULTICAST-DATOS
            {
                ...
                AddressEntryBasic entryBasic;
                bool entryExists = addressTableBasic->findEntryBasic(srcAddress, entryBasic);  //entryExists = findEntryBasic(srcAddress, entryBasic);
                if (!entryExists) //Not found, add address to bcast table
                {
                    EV << "      Entry not found -> Add new 'multicast' entry..." << endl;
                    addressTableBasic->addEntryBasic(entryBasic, srcAddress, inputport);//addEntryBasic(entryBasic, srcAddress, inputport);
...
//                            multicastFrame(frame, inputport);
    ...
    else
    {
        ...
        //Forwarding
        AddressEntry destEntry;
        bool entryExists = addressTable->findEntry(dstAddress, destEntry); //entryExists = findEntry(dstAddress, destEntry);
        if (entryExists)
        {
            EV << "      Output entry found at port: " << destEntry.portToSend << endl;
            EV << "        ...refresh destination + forward" << endl;
...
void MACRelayUnitWAPB::handleAndDispatchFrameV2Learning(EtherFrame* frame, MACAddress next_h)
{
    ...
    EV << "  Protocol version: 2 -> ARP-Path Learning Wireless (v2)" << endl;
        ...
        if(!strcmp("arpREQ",frame->getName()))
        {
            ...
            //Learning + Blocking/Broadcasting
            AddressEntry entry;
            entryExists = addressTable->findEntryw(srcAddress, entry);  //entryExists = findEntryw(srcAddress, entry);
            if (entryExists)
            {
                if(entry.status == locked)
                {
                    EV << "      Entry found -> 'locked' status..." << endl;
                    if (next_h == entry.next_hop)
                    {
                        EV << "        ...refresh" << endl;
                        entry.inTime = simTime(); //Refresco
                        addressTable->updateTable(srcAddress, entry);
                ...
                {
                    EV << "      Entry found -> 'learnt' status goes 'locked' as a new entry..." << endl;
                    addressTable->addEntryw(entry, srcAddress, next_h, protocolVersion);  //addEntryw(entry, srcAddress, next_h);
                    delete (frame);
                }
            }
            else
...
MACAddress MACRelayUnitWAPB::handleAndDispatchFrameV2Route(EtherFrame* frame)
{
    ...
    EV << "  Protocol version: 2 -> ARP-Path Route Unidirectional  (v2)" << endl;
        ...
        updateTables(); //Updates all entries from all the tables in the switch (aged entries, failed links, etc...)
           ...
           //4º) If not ARP but UNICAST (Unicast)
        if((!dstAddress.isBroadcast()) || (!dstAddress.isMulticast()))
           {
               EV << "    Frame is UNICAST" << endl;

               //Forwarding
               AddressEntry destEntry;
               entryExists = addressTable->findEntryw(dstAddress, destEntry);  //entryExists = findEntryw(dstAddress, destEntry);
               if (entryExists)
               {
                   EV << "      Output entry found at next-hop: " << destEntry.next_hop << endl;
                   EV << "        ...refresh destination + forward" << endl;
                   if(destEntry.status == learnt)
                   {
                       destEntry.inTime = simTime(); //Refresco sólo si la entrada ya está en estado 'learnt' (si no, estaríamos refrescando el estado 'locked' y eso no se hace con unicast)
                   ...
                   delete (frame);
                   return destEntry.next_hop;
               }
               else
               {
                   EV << "      Output entry not found" << endl;
                   delete (frame);
                   return noroute.UNSPECIFIED_ADDRESS;

               }
           }
        else
        {
            EV << "Frame is broadcast or multicast" << endl;
            delete (frame);
...
void MACRelayUnitWAPB::generateSwitchAddress()
{
...
void MACRelayUnitWAPB::generateParametersTablesAndEvents(void)
{
    std::string mac = "";
    std::string sport = "";
    size_t pos;

    EV << "->MACRelayUnitWAPB::generateParametersTables()" << endl;
...
void MACRelayUnitWAPB::updateTables()
{
    EV << "->MACRelayUnitWAPB::updateTables()" << endl;
    EV << "    Updating entries from LT (aged 'locked', 'learnt' and ports associated with down links entries):" << endl;
    addressTable->updateAndRemoveAgedEntriesFromLT();  //updateAndRemoveAgedEntriesFromLT();
    EV << "    Updating entries from BT (aged 'locked' and ports associated with down links entries):" << endl;
    addressTableBasic->updateAndRemoveAgedEntriesFromBT(); //updateAndRemoveAgedEntriesFromBT();
...
void MACRelayUnitWAPB::updateAndRemoveAgedEntriesFromHeT()
{
...
void MACRelayUnitWAPB::updateAndRemoveAgedEntriesFromRT()
{
...
void MACRelayUnitWAPB::removeAllEntriesFromTables()
{
    EV << "->MACRelayUnitWAPB::removeAllEntriesFromTables()" << endl;

    addressTable->clearLT();  //addressTable.clear();       //LT
    addressTableBasic->clearBT();  //addressTableBasic.clear();  //BT
    notHostList.clear();        //HeT
    repairTable.clear();        //RT
}

void MACRelayUnitWAPB::addEntryMulticast(AddressEntryMulticast entry, MACAddress address, int inputport, bool isNew)
{
...
bool MACRelayUnitWAPB::findEntryMulticast(MACAddress address, AddressEntryMulticast& entryMulticast)
{
    bool found = false;
...
bool MACRelayUnitWAPB::linkDownForPort(int port)
{
...
void MACRelayUnitWAPB::sendHello()
{
    EV << "->MACRelayUnitWAPB::sendHello()" << endl;
...
bool MACRelayUnitWAPB::inHeT(int port)
{
...
bool MACRelayUnitWAPB::inHoT(int port)
{
    if(inHeT(port))
        return false;
...
/*    for (AddressTable::iterator iter = addressTable.begin(); iter!=addressTable.end(); iter++)
        if(iter->second.port == port)
            return true;
*/
    if (addressTable->isPortInTable(port)) //new
        return true;
...
bool MACRelayUnitWAPB::inRepairTable(MACAddress address)
{
...
int MACRelayUnitWAPB::portOfHost(MACAddress address)
{
...
*/
    int port = addressTable->getPortByAddress(address);  //new
    if ((port != -1) && (!inHeT(port)))    //new
        return port;
...
EtherFrame* MACRelayUnitWAPB::repairFrame(RepairMessageType type, MACAddress srcAddress, MACAddress dstAddress, simtime_t repairTime)
{
    const char* name;
...
EtherFrame* MACRelayUnitWAPB::groupRepairFrame(RepairMessageType type, MACAddress repairSwitch, MACAddressVector addressVector, simtime_t repairTime)
{
    const char* name;
...
void MACRelayUnitWAPB::startPathRepair(MACAddress srcAddress, MACAddress dstAddress)
{
    ...
    if(repairType == 3) //No addresses given by the parameters and no need to check inRepairTable because the repair is just called once (when the link goes DOWN)
    {
        if(srcAddress.isUnspecified() || dstAddress.isUnspecified())
        {
          ...
          /*  for (AddressTable::iterator iter = addressTable.begin(); iter != addressTable.end(); iter++)   //old
            {
            ...
            */
            //new
            std::map<MACAddress, int> mapping;
            addressTable->getTableMapping(mapping);
            for (std::map<MACAddress, int>::iterator iter = mapping.begin(); iter != mapping.end(); iter++)
              {
                  EV << "  Address: " << iter->first;
                  if(linkDownForPort(iter->second))
                  {
                      EV << " -> Needs to be repaired (link is down!) -> Added to LinkFail";
                      addressVector.push_back(iter->first);
                  }
...
void MACRelayUnitWAPB::handlePathRepairFrame(EtherFrame *frame, int inputport)
{
    EV << "->MACRelayUnitWAPB::handlePathRepairFrame()" << endl;
    if(protocolVersion == 2)
...
//Implements the logic of "ARP-Path unidireccional condicionado (v3)"
void MACRelayUnitWAPB::handlePathRepairFrameV2(EtherFrame* frame, int inputport)
{
    ...
    else
    {
        ...
        else //Repair message
        {
            ...
            if (type == PathFail) //PathFail
            {
                ...
                entryExists = addressTableBasic->findEntryBasic(switchAddress, entryBasic); //entryExists = findEntryBasic(switchAddress, entryBasic);
                if (!entryExists) //Not found, add address to bcast table
                {
                    EV << "      Entry not found -> Add new 'broadcast' entry..." << endl;
                    addressTableBasic->addEntryBasic(entryBasic, switchAddress, inputport);  //addEntryBasic(entryBasic, switchAddress, inputport);
                }
                else //Address found
                {
                    EV << "      Entry was found..." << endl;
                    if (entryBasic.port == inputport) //If port is coincident -> refresh entry
                    {
                        EV << "        ...refresh!" << endl;
                        entryBasic.inTime = simTime();
                        addressTableBasic->updateTableBasic(switchAddress, entryBasic); //addressTableBasic[switchAddress] = entryBasic;
                    }
                    else //Else -> discard
                    {
                        EV << "        ...discard!" << endl;
            ...
            else if (type == PathRequest) //PathRequest
            {
                ...
                if(!discardFrame)
                {
                    ...
                    if (port !=-1 /*&& entry.port != -1*/) //If dst directly connected, send PathReply only if applicable (i.e. version 1)
                    {
                         ...
                         if(repairType == 1)
                         {
                             entryExists = addressTable->findEntry(dstAddress, entry);  //entryExists = findEntry(dstAddress, entry);
                             entry.inTime = simTime();
                             entry.status = locked;     //Update entry to 'locked' (the learning process starts again)
...
//Implements the logic of "ARP-Path unidireccional condicionado (v3)"
void MACRelayUnitWAPB::handlePathRepairFrameV3(EtherFrame* frame, int inputport)
{
    PathRepair* prf = check_and_cast<PathRepair*>(frame);
    MACAddress switchAddress = prf->getSrc();    //MAC address/ID of the message's source switch
    unsigned int type = prf->getType();
    ...
    else
    {
        ...
        else //Repair message
        {
            ...
            else if (type == LinkFail) //LinkFail
            {
                ...
                entryExists = addressTable->findEntry(switchAddress, entry);  //entryExists = findEntry(switchAddress, entry);
                if (entryExists)
                {
                    if(entry.status == locked)
                    {
                        ...
                        if (inputport == entry.port)
                        {
                            EV << "        ...refresh!" << endl;
                            entry.inTime = simTime(); //Refresco
                            addressTable->updateTable(switchAddress, entry);  //addressTable[switchAddress] = entry;
                        }
                        else
                        {
                            EV << "        ...discard!" << endl;
                    ...
                    else
                    {
                        EV << "      Entry found -> 'learnt' status goes 'locked' as a new entry..." << endl;
                        entry.portToSend = -1; //#V3 - En v3 se resetea el portToSend a -1 porque si no conservaría el valor viejo tras 'addEntry'
                        addressTable->addEntry(entry, switchAddress, inputport, protocolVersion);
                    }
                }
                else
                {
                ...
                if(!discardFrame)
                {
                    ...
                    for(unsigned int i=0; i<addressVector.size(); i++)
                    {
                        ...
                        if (port != -1)
                        {
                            ...
                            entryExists = addressTable->findEntry(addressVector[i], entry);  //entryExists = findEntry(addressVector[i], entry);
                            entry.inTime = simTime();
                            entry.status = locked;     //Update entry to 'locked' (the learning process starts again)
                            addressTable->updateTable(srcAddress, entry);
            ...
            else if (type == LinkReply) //LinkReply
            {
                ...
                for(unsigned int i=0; i<addressVector.size(); i++)
                {
                    ...
                    if (entryExists)
                    {
                        if(entry.status == locked)
                        {
                            ...
                            {
                                entry.inTime = simTime(); //Refresco
                                addressTable->updateTable(addressVector[i], entry); //addressTable[addressVector[i]] = entry;
                            }
                            //else //No hacemos nada si no coincide (forward as if entry port was the same, but no refresh)
                        }
                        else
                        {
                            EV << "      Entry found -> 'learnt' status goes 'locked' as a new entry..." << endl;
                            entry.portToSend = -1; //#V3 - En v3 se resetea el portToSend a -1 porque si no conservaría el valor viejo tras 'addEntry'
                            addressTable->addEntry(entry, addressVector[i], inputport, protocolVersion);
                        }
                    }
                    else
                    {
                ...
                {
                    AddressEntry destEntry;
                    entryExists = addressTable->findEntry(repairSwitch, destEntry);  //entryExists = findEntry(repairSwitch, destEntry); //To the 'LinkFail switch'
                    if (entryExists)
                    {
                        EV << "      Output entry found at port: " << destEntry.port << endl;
                        EV << "        ...refresh destination + forward" << endl;
...
void MACRelayUnitWAPB::updateRepairStatistics(PathRepair* prframe, unsigned int type, unsigned int nRepair)
{
...
void MACRelayUnitWAPB::multicastFrame(EtherFrame *frame, int inputport, AddressEntryMulticast entry)
{
...
void MACRelayUnitWAPB::broadcastFrame(EtherFrame *frame, int inputport, bool repairMessage)
{
    std::vector<int> sequence;
    generateSendSequence(sequence);
...
void MACRelayUnitWAPB::generateSendSequence(std::vector<int>& sequence)
{
    int r_iter;
...
void MACRelayUnitWAPB::printTables()
{
    addressTable->printLT();
    addressTableBasic->printBT();
    printHeT();
    printRT();
}

//Method to print the contents of HeT
void MACRelayUnitWAPB::printHeT()
{
    EV << endl;
    EV << "    HeT (" << this->getParentModule()->getFullName() << ") (" << notHostList.size() << " entries):\n";
    EV << "      ---------" << endl;
...
//Print linkDownTable
void MACRelayUnitWAPB::printLinkDownTable(void)
{
...
void MACRelayUnitWAPB::printSwitchDownStruct(void)
{
    //switchDownStruct
    EV << endl;
...
void MACRelayUnitWAPB::printRouteStatistics()
{
    EV << "->MACRelayUnitWAPB::printRouteStatistics()" << endl;
