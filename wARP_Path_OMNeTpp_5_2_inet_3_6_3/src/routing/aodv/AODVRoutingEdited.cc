...
#include "src/routing/aodv/AODVRoutingEdited.h"
...
Define_Module(AODVRoutingEdited);

void AODVRoutingEdited::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL) {
        ...
        WATCH(numRREQCreated);
        WATCH(numRREPCreated);
        WATCH(numRERCreated);
        WATCH(numGratuitousRREPCreated);

        WATCH(numRREQForwarded);
        WATCH(numRREPForwarded);
        WATCH(numRERForwarded);
        WATCH(numGratuitousRREPForwarded);

        WATCH(numStartRouteDiscovery);
...
void AODVRoutingEdited::handleMessage(cMessage *msg)
{
    if (!isOperational) {
...
INetfilter::IHook::Result AODVRoutingEdited::ensureRouteForDatagram(INetworkDatagram *datagram)
{
...
AODVRoutingEdited::AODVRoutingEdited()
{
}

bool AODVRoutingEdited::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void AODVRoutingEdited::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
    ...
    numStartRouteDiscovery ++;
}

L3Address AODVRoutingEdited::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void AODVRoutingEdited::delayDatagram(INetworkDatagram *datagram)
{
...
void AODVRoutingEdited::sendRREQ(AODVRREQ *rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AODVRoutingEdited::sendRREP(AODVRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
AODVRREQ *AODVRoutingEdited::createRREQ(const L3Address& destAddr)
{
    ...
    numRREQCreated++;  //EXTRA
    return rreqPacket;
}

AODVRREP *AODVRoutingEdited::createRREP(AODVRREQ *rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    ...
    numRREPCreated++;  //EXTRA
    return rrep;
}

AODVRREP *AODVRoutingEdited::createGratuitousRREP(AODVRREQ *rreq, IRoute *originatorRoute)
{
    ...
    numGratuitousRREPCreated++; //EXTRA
    return grrep;
}

void AODVRoutingEdited::handleRREP(AODVRREP *rrep, const L3Address& sourceAddr)
{
...
void AODVRoutingEdited::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void AODVRoutingEdited::sendAODVPacket(AODVControlPacket *packet, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void AODVRoutingEdited::handleRREQ(AODVRREQ *rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    //if (checkRREQArrivalTime != rreqsArrivalTime.end() && (simTime() - checkRREQArrivalTime->second <= pathDiscoveryTime)) {
        ...
        //EXTRA
        EV_WARN << "rreqIdentifier : getOriginatorAddr = " << rreqIdentifier.originatorAddr << " getRreqId = " << rreqIdentifier.rreqID << endl;
        EV_WARN << "RREQ : getOriginatorAddr = " << rreq->getOriginatorAddr() << " getRreqId = " << rreq->getRreqId() << endl;
        EV_WARN <<"entry found: checkRREQArrivalTime->first.originatorAddr = "<< checkRREQArrivalTime->first.originatorAddr  << " checkRREQArrivalTime->first.rreqID = " << checkRREQArrivalTime->first.rreqID<< "checkRREQArrivalTime->second = " << checkRREQArrivalTime->second <<" size = "<<rreqsArrivalTime.size() << endl;
        auto itertest= rreqsArrivalTime.begin();
        for (auto itertest= rreqsArrivalTime.begin(); itertest != rreqsArrivalTime.end(); itertest++){
            EV_WARN << "table entries: rreqsArrivalTime->first.originatorAddr = " << itertest->first.originatorAddr << "rreqsArrivalTime->first.rreqID" << itertest->first.rreqID << endl;
    ...
    if (timeToLive > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0)) {
        ...
        //EXTRA
        EV_INFO << "aaa getOriginatorAddr = " << rreq->getOriginatorAddr()<< " getRreqId = " << rreq->getRreqId() << endl;
...
IRoute *AODVRoutingEdited::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void AODVRoutingEdited::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void AODVRoutingEdited::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
AODVRERR *AODVRoutingEdited::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void AODVRoutingEdited::handleRERR(AODVRERR *rerr, const L3Address& sourceAddr)
{
...
bool AODVRoutingEdited::handleOperationStage(LifecycleOperation *operation, int stage, IDoneCallback *doneCallback)
{
    Enter_Method_Silent();
...
void AODVRoutingEdited::clearState()
{
...
void AODVRoutingEdited::handleWaitForRREP(WaitForRREP *rrepTimer)
{
...
void AODVRoutingEdited::forwardRREP(AODVRREP *rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void AODVRoutingEdited::forwardRREQ(AODVRREQ *rreq, unsigned int timeToLive)
{
...
void AODVRoutingEdited::completeRouteDiscovery(const L3Address& target)
{
...
void AODVRoutingEdited::sendGRREP(AODVRREP *grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
AODVRREP *AODVRoutingEdited::createHelloMessage()
{
...
void AODVRoutingEdited::sendHelloMessagesIfNeeded()
{
...
void AODVRoutingEdited::handleHelloMessage(AODVRREP *helloMessage)
{
...
void AODVRoutingEdited::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void AODVRoutingEdited::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result AODVRoutingEdited::datagramForwardHook(INetworkDatagram *datagram, const InterfaceEntry *inputInterfaceEntry, const InterfaceEntry *& outputInterfaceEntry, L3Address& nextHopAddress)
{
...
void AODVRoutingEdited::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void AODVRoutingEdited::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool AODVRoutingEdited::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AODVRouteData *routeData = check_and_cast<AODVRouteData *>(route->getProtocolData());
...
AODVRREPACK *AODVRoutingEdited::createRREPACK()
{
...
void AODVRoutingEdited::sendRREPACK(AODVRREPACK *rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void AODVRoutingEdited::handleRREPACK(AODVRREPACK *rrepACK, const L3Address& neighborAddr)
{
...
void AODVRoutingEdited::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void AODVRoutingEdited::handleBlackListTimer()
{
