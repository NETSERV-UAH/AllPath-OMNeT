...
void EtherMACFullDuplexNew::initialize(int stage)
{
...
void EtherMACFullDuplexNew::processFrameFromUpperLayer(EtherFrame *frame)
{
    ...
    //our change
    if (!connected || disabled) // if ((!connected || disabled)||((strcmp(this->getParentModule()->getParentModule()->getNedTypeName(),"inet.node.inet.StandardHost")==0)&&((strcmp(frame->getName(),"LinkFail")==0)||(strcmp(frame->getName(),"LinkReply")==0))))
    {
...
void EtherMACFullDuplexNew::processMsgFromNetwork(cPacket *pk)
{
    ...
    if ((!connected || disabled)||((strcmp(this->getParentModule()->getParentModule()->getNedTypeName(),"inet.node.inet.StandardHost")==0)&&((strcmp(msg->getName(),"LinkFail")==0)||(strcmp(msg->getName(),"LinkReply")==0))))
...
MACAddress EtherMACFullDuplexNew::getAddress()
{
    return address;
}
