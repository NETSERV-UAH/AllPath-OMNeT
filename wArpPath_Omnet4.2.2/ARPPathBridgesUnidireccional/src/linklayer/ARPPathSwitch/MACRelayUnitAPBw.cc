...
Define_Module(MACRelayUnitAPBw);

//class constructor
MACRelayUnitAPBw::MACRelayUnitAPBw()
{
    endProcEvents = NULL;
    numCPUs = 0;
}
...
void MACRelayUnitAPBw::initialize()
{
    /*MACRelayUnitNP.h*/
...
void MACRelayUnitAPBw::handleMessage(cMessage *msg)
{
    EV << "->MACRelayUnitAPBw::handleMessage()" << endl;
...
void MACRelayUnitAPBw::handleIncomingFrame(EtherFrame *frame)
{
...
void MACRelayUnitAPBw::processFrame(cMessage *msg)
{
    int cpu = msg->getKind();
...
void MACRelayUnitAPBw::handleAndDispatchFrame(EtherFrame *frame, int inputport)
{
    EV << "->MACRelayUnitAPBw::handleAndDispatchFrame()" << endl;
...
void MACRelayUnitAPBw::handleAndDispatchFrameV2(EtherFrame *frame, int inputport)
{
...
void MACRelayUnitAPBw::handleAndDispatchFrameV3(EtherFrame* frame, int inputport)
{
...
void MACRelayUnitAPBw::handleAndDispatchFrameV2Learning(EtherFrame* frame, MACAddress next_h)
{
    EV << "  Protocol version: 2 -> ARP-Path Learning Wireless (v2)" << endl;
        ...
        if(!strcmp("arpREQ",frame->getFullName()))
        {
            ...
            //Learning + Blocking/Broadcasting
            AddressEntry entry;
            entryExists = findEntryw(srcAddress, entry);
            if (entryExists)
            {
                if(entry.status == locked)
                {
                ...
                {
                    EV << "      Entry found -> 'learnt' status goes 'locked' as a new entry..." << endl;
                    addEntryw(entry, srcAddress, next_h);
                    delete (frame);
                }
            }
            else
...
MACAddress MACRelayUnitAPBw::handleAndDispatchFrameV2Route(EtherFrame* frame)
{
        ...
        if((!dstAddress.isBroadcast()) || (!dstAddress.isMulticast()))
           {
               ...
               //Forwarding
               AddressEntry destEntry;
               entryExists = findEntryw(dstAddress, destEntry);
               if (entryExists)
               {
...
void MACRelayUnitAPBw::generateSwitchAddress()
{
...
void MACRelayUnitAPBw::generateParametersTablesAndEvents(void)
{
    std::string mac = "";
    std::string sport = "";
    size_t pos;

    EV << "->MACRelayUnitAPBw::generateParametersTables()" << endl;
...
void MACRelayUnitAPBw::updateTables()
{
    EV << "->MACRelayUnitAPBw::updateTables()" << endl;
...
void MACRelayUnitAPBw::updateAndRemoveAgedEntriesFromLT()
{
...
void MACRelayUnitAPBw::updateAndRemoveAgedEntriesFromBT()
{
...
void MACRelayUnitAPBw::updateAndRemoveAgedEntriesFromHeT()
{
...
void MACRelayUnitAPBw::updateAndRemoveAgedEntriesFromRT()
{
...
void MACRelayUnitAPBw::removeAllEntriesFromTables()
{
    EV << "->MACRelayUnitAPBw::removeAllEntriesFromTables()" << endl;
...
void MACRelayUnitAPBw::addEntryw(AddressEntry entry, MACAddress address, MACAddress next_h)
{
...
//Method to add an entry in the special broadcast address table, given its MAC address and next_hop
void MACRelayUnitAPBw::addEntryBasicw(AddressEntryBasic entry, MACAddress address, MACAddress next_h)
{
    entry.inTime = simTime();
    entry.next_hop = next_h;
...
void MACRelayUnitAPBw::addEntry(AddressEntry entry, MACAddress address, int inputport)
{
    entry.inTime = simTime();
...
void MACRelayUnitAPBw::addEntryBasic(AddressEntryBasic entry, MACAddress address, int inputport)
{
    entry.inTime = simTime();
    entry.port = inputport;
    addressTableBasic[address] = entry;
...
void MACRelayUnitAPBw::addEntryMulticast(AddressEntryMulticast entry, MACAddress address, int inputport, bool isNew)
{
    entry.inTime = simTime();
...
bool MACRelayUnitAPBw::findEntry(MACAddress address, AddressEntry& entry)
{
    bool found = false;
...
bool MACRelayUnitAPBw::findEntryBasic(MACAddress address, AddressEntryBasic& entryBasic)
{
    bool found = false;
...
//Method to find an entry in the address table, given its MAC address.
bool MACRelayUnitAPBw::findEntryw(MACAddress address, AddressEntry& entry)
{
    bool found = false;
    AddressTable::iterator iter = addressTable.find(address);
...
//Method to find an entry in the special broadcast address table, given its MAC address.
bool MACRelayUnitAPBw::findEntryBasicw(MACAddress address, AddressEntryBasic& entryBasic)
{
    bool found = false;
    AddressTableBasic::iterator iter = addressTableBasic.find(address);
...
bool MACRelayUnitAPBw::findEntryMulticast(MACAddress address, AddressEntryMulticast& entryMulticast)
{
    bool found = false;
...
bool MACRelayUnitAPBw::linkDownForPort(int port)
{
...
void MACRelayUnitAPBw::sendHello()
{
    EV << "->MACRelayUnitAPBw::sendHello()" << endl;
...
bool MACRelayUnitAPBw::inHeT(int port)
{
...
bool MACRelayUnitAPBw::inHoT(int port)
{
    if(inHeT(port))
        return false;
...
bool MACRelayUnitAPBw::inRepairTable(MACAddress address)
{
...
int MACRelayUnitAPBw::portOfHost(MACAddress address)
{
    AddressTable::iterator iter = addressTable.find(address);
...
EtherFrame* MACRelayUnitAPBw::repairFrame(RepairMessageType type, MACAddress srcAddress, MACAddress dstAddress, simtime_t repairTime)
{
    const char* name;
...
EtherFrame* MACRelayUnitAPBw::groupRepairFrame(RepairMessageType type, MACAddress repairSwitch, MACAddressVector addressVector, simtime_t repairTime)
{
    const char* name;
...
void MACRelayUnitAPBw::startPathRepair(MACAddress srcAddress, MACAddress dstAddress)
{
...
void MACRelayUnitAPBw::handlePathRepairFrame(EtherFrame *frame, int inputport)
{
    EV << "->MACRelayUnitAPBw::handlePathRepairFrame()" << endl;
    if(protocolVersion == 2)
...
//Implements the logic of "ARP-Path unidireccional condicionado (v3)"
void MACRelayUnitAPBw::handlePathRepairFrameV2(EtherFrame* frame, int inputport)
{
...
//Implements the logic of "ARP-Path unidireccional condicionado (v3)"
void MACRelayUnitAPBw::handlePathRepairFrameV3(EtherFrame* frame, int inputport)
{
    PathRepair* prf = check_and_cast<PathRepair*>(frame);
    MACAddress switchAddress = prf->getSrc();    //MAC address/ID of the message's source switch
    unsigned int type = prf->getType();
...
void MACRelayUnitAPBw::updateRepairStatistics(PathRepair* prframe, unsigned int type, unsigned int nRepair)
{
...
void MACRelayUnitAPBw::multicastFrame(EtherFrame *frame, int inputport, AddressEntryMulticast entry)
{
...
void MACRelayUnitAPBw::broadcastFrame(EtherFrame *frame, int inputport, bool repairMessage)
{
    std::vector<int> sequence;
    generateSendSequence(sequence);
...
void MACRelayUnitAPBw::generateSendSequence(std::vector<int>& sequence)
{
    int r_iter;
...
int MACRelayUnitAPBw::getNumberOfEntriesForPort(int port){
...
void MACRelayUnitAPBw::printTables()
{
    printLT();
    printBT();
    printHeT();
...
void MACRelayUnitAPBw::printHeT()
{
    EV << endl;
    EV << "    HeT (" << this->getParentModule()->getFullName() << ") (" << notHostList.size() << " entries):\n";
    EV << "      ---------" << endl;
...
//Print linkDownTable
void MACRelayUnitAPBw::printLinkDownTable(void)
{
...
void MACRelayUnitAPBw::printSwitchDownStruct(void)
{
    //switchDownStruct
    EV << endl;
...
void MACRelayUnitAPBw::printRouteStatistics()
{
    EV << "->MACRelayUnitAPBw::printRouteStatistics()" << endl;
