 ...
 * Copyright (C) 2014 Andrés Beato (forwarding ARP-REQ in AdhocHostAPB)
 *
...
void ARPNew::sendPacketToNIC(cMessage *msg, InterfaceEntry *ie, const MACAddress& macAddress, int etherType)
{
    ...
    MACAddress myMACAddress = ie->getMacAddress();//*ANDRES*
    Ieee802Ctrl *controlInfo = new Ieee802Ctrl();
    controlInfo->setSrc(myMACAddress);//*ANDRES*
...
void ARPNew::sendPacketToNICForwarding(cMessage *msg, InterfaceEntry *ie, const MACAddress& macAddress,const MACAddress& macSRCAddress, int etherType)
{
    // add control info with MAC address
    Ieee802Ctrl *controlInfo = new Ieee802Ctrl();
    controlInfo->setSrc(macSRCAddress);
    controlInfo->setDest(macAddress);
    controlInfo->setEtherType(etherType);
    msg->setControlInfo(controlInfo);
...
void ARPNew::sendARPRequest(InterfaceEntry *ie, IPv4Address ipAddress)
{
    ...
    arp->setDestMACAddress(broadcastAddress);//*ANDRES*
...
void ARPNew::processARPPacket(ARPPacketRoute *arp)
{
    ...
    MACAddress dest_Address = arp->getDestMACAddress();//*ANDRES*
    ...
    else
    {
        ...
        // *ANDRES*
        std::string modAdHoc = "AdhocHostAPB";
        if(getParentModule()->getParentModule()->getSubmodule("relayUnit"))
        {
            /*MACAddress myMACAddress = ie->getMacAddress();
            if (srcMACAddress ==myMACAddress)
            {
                EV << "Is my own ARP-Resolution in AdHoc mode, delete ARP(AdhocHostAPB) "<< "\n";
                delete arp;
            }
            else
            {*/
            //reenviamos el ARP-REQ
            static MACAddress broadcastAddress("ff:ff:ff:ff:ff:ff");
            sendPacketToNICForwarding(arp, ie, dest_Address,srcMACAddress, ETHERTYPE_ARP);
            EV << "Address not recognized, forwarding (AdhocHostAPB) "<< "\n";
            //}
        }
        // *ANDRES*
        else
