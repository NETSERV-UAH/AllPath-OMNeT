...
void IPv4New::updateDisplayString()
{
    char buf[80] = "";
...
void IPv4New::endService(cPacket *msg)
{
    ...
    else if (dynamic_cast<ARPPacketRoute *>(msg)) //EXTRA
    {
        ...
        handleARP((ARPPacketRoute *)msg); //EXTRA
    }
    else
    {
...
InterfaceEntry *IPv4New::getSourceInterfaceFrom(cPacket *msg)
{
    cGate *g = msg->getArrivalGate();
    return g ? ift->getInterfaceByNetworkLayerGateIndex(g->getIndex()) : NULL;
}

void IPv4New::handlePacketFromNetwork(IPv4Datagram *datagram, InterfaceEntry *fromIE)
{
    ASSERT(datagram);
    ASSERT(fromIE);
...
void IPv4New::handleARP(ARPPacketRoute *msg)
{
...
void IPv4New::handleReceivedICMP(ICMPMessage *msg)
{
    switch (msg->getType())
    {
...
void IPv4New::handleMessageFromHL(cPacket *msg)
{
 ...
 */
InterfaceEntry *IPv4New::determineOutgoingInterfaceForMulticastDatagram(IPv4Datagram *datagram, InterfaceEntry *multicastIFOption)
{
...
void IPv4New::routeUnicastPacket(IPv4Datagram *datagram, InterfaceEntry *destIE, IPv4Address destNextHopAddr)
{
    IPv4Address destAddr = datagram->getDestAddress();
...
void IPv4New::routeLocalBroadcastPacket(IPv4Datagram *datagram, InterfaceEntry *destIE)
{
...
InterfaceEntry *IPv4New::getShortestPathInterfaceToSource(IPv4Datagram *datagram)
{
    return rt->getInterfaceForDestAddr(datagram->getSrcAddress());
}

void IPv4New::forwardMulticastPacket(IPv4Datagram *datagram, InterfaceEntry *fromIE)
{
    ASSERT(fromIE);
...
void IPv4New::reassembleAndDeliver(IPv4Datagram *datagram)
{
    EV << "Local delivery\n";
...
cPacket *IPv4New::decapsulate(IPv4Datagram *datagram)
{
...
void IPv4New::fragmentAndSend(IPv4Datagram *datagram, InterfaceEntry *ie, IPv4Address nextHopAddr)
{
...
IPv4Datagram *IPv4New::encapsulate(cPacket *transportPacket, IPv4ControlInfo *controlInfo)
{
...
IPv4Datagram *IPv4New::createIPv4Datagram(const char *name)
{
    return new IPv4Datagram(name);
}

void IPv4New::sendDatagramToOutput(IPv4Datagram *datagram, InterfaceEntry *ie, IPv4Address nextHopAddr)
{
    if (ie->isLoopback())
    {
...
#ifdef WITH_MANET
void IPv4New::sendRouteUpdateMessageToManet(IPv4Datagram *datagram)
{
...
void IPv4New::sendNoRouteMessageToManet(IPv4Datagram *datagram)
{
    if (datagram->getTransportProtocol()==IP_PROT_DSR)
    {
        sendToManet(datagram);
...
void IPv4New::sendToManet(cPacket *packet)
{
    ASSERT(manetRouting);
    int gateindex = mapping.getOutputGateForProtocol(IP_PROT_MANET);
